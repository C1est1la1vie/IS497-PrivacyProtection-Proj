...
class DataManageUI(QWidget):
    ...
    def __init__(self):
        ...
        loadUi('./identity/ui/DataManage.ui', self)
        self.setWindowIcon(QIcon('../identity/icons/icon.png'))
        self.setFixedSize(931, 577)
        self.setWindowTitle('pc端个人隐私防护系统 - 模型建立')
    ...
    def queryUser(self):
        ...
        try:
            ...
            self.queryUserButton.setIcon(QIcon('../identity/icons/error.png'))
            ...
            self.queryResultLabel.clear()
            self.queryUserButton.setIcon(QIcon('../identity/icons/success.png'))
    ...
    def deleteUser(self):
        ...
        if ret == QMessageBox.Yes:
            ...
            try:
                ...
                self.deleteUserButton.setIcon(QIcon('../identity/icons/error.png'))
                ...
                self.initDb()
                self.deleteUserButton.setIcon(QIcon('../identity/icons/success.png'))
    ...
    def detectFace(self, img):
        ...
        face_cascade = cv2.CascadeClassifier('./identity/haarcascades/haarcascade_frontalface_default.xml')
    ...
    def train(self):
        try:
            ...
            if ret == QMessageBox.Yes:
                ...
                if not os.path.exists('./identity/recognizer'):
                    os.makedirs('./identity/recognizer')
            faces, labels = self.prepareTrainingData(self.datasets)
            face_recognizer.train(faces, np.array(labels))
            face_recognizer.save('./identity/recognizer/trainingData.yml')
        except FileNotFoundError:
            logging.error('系统找不到人脸数据目录{}'.format(self.datasets))
            self.trainButton.setIcon(QIcon('../identity/icons/error.png'))
            ...
            DataManageUI.callDialog(QMessageBox.Information, text, informativeText, QMessageBox.Ok)
            self.trainButton.setIcon(QIcon('../identity/icons/success.png'))
    ...
    def callDialog(icon, text, informativeText, standardButtons, defaultButton=None):
        ...
        msg.setWindowTitle('pc端个人隐私防护系统 - 模型建立')
        msg.setIcon(icon)
        msg.setText(text)
