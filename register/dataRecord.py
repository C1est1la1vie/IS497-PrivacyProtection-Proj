...
class DataRecordUI(QWidget):
    ...
    def __init__(self):
        ...
        self.faceCascade = cv2.CascadeClassifier('./identity/haarcascades/haarcascade_frontalface_default.xml')
    ...
    def startWebcam(self, status):
        if status:
            ...
            if not ret:
                ...
                self.timer.start(5)
                self.startWebcamButton.setIcon(QIcon('../identity/icons/success.png'))
        else:
        ...
        if startFaceRecordButton.text() == '开始采集人脸数据':
            if self.isFaceDetectEnabled:
                if self.isUserInfoReady:
                    ...
                    self.startFaceRecordButton.setIcon(QIcon('../identity/icons/success.png'))
                    self.startFaceRecordButton.setText('结束当前人脸采集')
                else:
                    self.startFaceRecordButton.setIcon(QIcon('../identity/icons/error.png'))
        ...
        for (x, y, w, h) in faces:
            if self.isFaceRecordEnabled:
                try:
                    ...
                    self.enableFaceRecordButton.setIcon(QIcon('../identity/icons/warning.png'))
                    continue
                except Exception as e:
                    logging.error('写入人脸图像文件到计算机过程中发生异常')
                    self.enableFaceRecordButton.setIcon(QIcon('../identity/icons/error.png'))
                    self.logQueue.put('Error：无法保存人脸图像，采集当前捕获帧失败')
                else:
                    self.enableFaceRecordButton.setIcon(QIcon('../identity/icons/success.png'))
    ...
    def migrateToDb(self):
        if self.isFaceDataReady:
            ...
            try:
                ...
                self.migrateToDbButton.setIcon(QIcon('../identity/icons/error.png'))
                ...
                self.migrateToDbButton.setIcon(QIcon('../identity/icons/success.png'))
...
class UserInfoDialog(QDialog):
    def __init__(self):
        ...
        loadUi('./identity/ui/UserInfoDialog.ui', self)
        self.setWindowIcon(QIcon('../identity/icons/icon.png'))
